...
export default function AuctionDetails() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [aRes, bRes] = await Promise.all([
        fetch(`https://das-backend-final.onrender.com/api/auctions/${id}/`),
        fetch(`https://das-backend-final.onrender.com/api/auctions/${id}/bid/`),
      ]);
      ...
      if (token && username) {
        const rRes = await fetch(
          `https://das-backend-final.onrender.com/api/auctions/${id}/ratings/`,
          { headers }
        );
        if (rRes.ok) {
      ...
      const cRes = await fetch(
        `https://das-backend-final.onrender.com/api/auctions/${id}/comments/`
      );
  ...
  const handleBidSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://das-backend-final.onrender.com/api/auctions/${id}/bid/`,
        {
          method: "POST",
  ...
  const sendRating = async (v) => {
    ...
    const url = editing
      ? `https://das-backend-final.onrender.com/api/auctions/${id}/ratings/${userRating.id}/`
      : `https://das-backend-final.onrender.com/api/auctions/${id}/ratings/`;
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://das-backend-final.onrender.com/api/auctions/${id}/comments/${editingComment}/`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json", ...headers },
          body: JSON.stringify({
  ...
  const handleDelete = async (commentId) => {
    try {
      const res = await fetch(
        `https://das-backend-final.onrender.com/api/auctions/${id}/comments/${commentId}/`,
        { method: "DELETE", headers }
      );
      if (!res.ok) throw new Error(await res.text());
      fetchAll();
